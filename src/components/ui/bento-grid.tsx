  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 p-4 max-w-7xl mx-auto">
      {items.map((item, index) => (
        ...
        >
          <div
            className={`${item.hasPersistentHover ? "opacity-100" : "opacity-0 group-hover:opacity-100"} absolute inset-0 transition-opacity duration-300`}
          >
          ...
          <div
            className={`${item.hasPersistentHover ? "opacity-100" : "opacity-0 group-hover:opacity-100"} absolute inset-0 -z-10 rounded-xl p-px bg-gradient-to-br from-transparent via-gray-100/50 to-transparent dark:via-white/10 transition-opacity duration-300`}
          />
        </div>
      ))}
    </div>
